...
n_final_files_per_sample = 2  # how many merged output files per sample
# ---------------------------
...
def merge_outputs():
    """Merge all _part*.root files into one or more _s2.root per sample."""
    ...
    for filename in os.listdir(input_dir):
        ...
        # Split parts into groups if user wants multiple final files
        n_groups = max(1, n_final_files_per_sample)
        group_size = (len(part_files) + n_groups - 1) // n_groups

        for i in range(n_groups):
            group = part_files[i * group_size : (i + 1) * group_size]
            if not group:
                continue

            merged_file = (
                os.path.join(output_dir, f"{base}_s2.root")
                if n_groups == 1
                else os.path.join(output_dir, f"{base}_s2_{i}.root")
            )

            print(f"Merging {len(group)} parts → {merged_file}")
            cmd = ["hadd", "-f", merged_file] + group
            result = subprocess.run(cmd, capture_output=True, text=True)

            if result.returncode != 0:
                print(f"[ERROR] Failed to merge {base} group {i}: {result.stderr.strip()}")
            else:
                print(f"[OK] {base} group {i} merged successfully.")
